...
async function figure_out_who(item: string, source: boolean): Promise<string[]> {
  ...
  if (alv != null) {
    return ["node", source? alv.source ?? "non-existent-source" : alv.sink ?? "non-existent-sink"]
  }

  let ext = extname(item)
  alv = file_extensions.map.get(ext)
  if (alv != null) {
    return ["node", source? alv.source ?? "non-existent-source" : alv.sink ?? "non-existent-sink", item]
  }
